...
def generate_list_calls(nfts_for_listing, ksm_price, version, batch_amount=list_batch_amount):
    rmrk_calls = []
    list_calls = []
...
def generate_send_calls(nfts_to_send_dict, version, batch_amount=send_batch_amount):
    rmrk_calls = []
    send_calls = []
...
def generate_emote_calls(nfts_to_emote, emote_list, version, batch_amount=emote_batch_amount):
    rmrk_calls = []
    emote_calls = []
...
def generate_mint_calls(nfts_info_to_mint, version, recipient="", batch_amount=mint_batch_amount):
    rmrk_calls = []
    mint_calls = []
...
def send_send_extrinsics(nfts_to_send_dict, version, keypair, batch_amount=send_batch_amount):
    generated_calls = generate_send_calls(nfts_to_send_dict, version, batch_amount)
    send_generated_calls(generated_calls, keypair)


def send_emote_extrinsics(nfts_to_emote, emote_list, version, keypair, batch_amount=emote_batch_amount):
    generated_calls = generate_emote_calls(nfts_to_emote, emote_list, version, batch_amount)
    send_generated_calls(generated_calls, keypair)


def send_list_extrinsics(nfts_for_listing, ksm_price, version, keypair, batch_amount=list_batch_amount):
    generated_calls = generate_list_calls(nfts_for_listing, ksm_price, version, batch_amount)
    send_generated_calls(generated_calls, keypair)


def send_mint_extrinsics(nfts_info_to_mint, version, keypair, recipient="", batch_amount=mint_batch_amount):
    generated_calls = generate_mint_calls(nfts_info_to_mint, version, recipient, batch_amount)
    send_generated_calls(generated_calls, keypair)
...
def send_buy_extrinsic(nft_id, address, ksm_price, version, keypair, send_fee=False):
    rmrk_calls = [{
        'call_module': 'System',
        'call_function': 'remark',
...
def mint_collection(name, symbol, metadata, version, keypair, max_amount=0, recipient=""):
    mint_json = {
        "name": name,
        "max": max_amount,
